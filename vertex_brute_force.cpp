...
void generate_output_file(int &min_cover_size, set<int> best_cover, double time_taken)
{
    ...
    if (!outfile.is_open())
    {
        cout << "Error: Could not open output.txt for writing.\n";
        return;
    }
...
void brute_force_vertex_cover(int n, const vector<pair<int, int>> &edges)
{
    ...
    for (int size = 1; size <= n; size++)
    {
        ...
        if (min_cover_size == size)
            break; // If we found the smallest vertex cover, stop
    }
...
int main()
{
    ...
    int result = system("python graph.py generate input.txt");
    if (result != 0)
    {
    ...
    cout << "\nDrawing Graph..." << endl;
    ;
    result = system("python graph.py draw input.txt");
    if (result != 0)
    {
    ...
    result = system("python graph.py vc input.txt output.txt");
    if (result != 0)
    {
        cout << "error running vertex cover graph Python script" << endl;
        return 1;
    }
